...
public class ConfigManager {
     ...
     */
    public static Config.ConfigImpl impl;
    ...
    /**
     * 读取配置文件，并将读取到的配置选项的值赋值给 {@link ConfigManager#impl} 实例。期间会检查并迁移旧版配置文件，并自动替换无效值。
     */
    public static void loadConfig() {
        if (!CONFIG_FILE.exists()) {
            ...
            if (Files.exists(FabricLoader.getInstance().getConfigDir().resolve("ultimatescaler.toml"))) {
                // 存在旧版配置文件，迁移配置
                impl = new Toml().read(FabricLoader.getInstance().getConfigDir().resolve("ultimatescaler.toml").toFile()).to(Config.ConfigImpl.class);
                try {
            ...
            // 不存在任何配置文件，使用默认配置
            impl = new Config.ConfigImpl();
        } else {
            try {
                // 存在配置文件，尝试读取配置
                impl = new Toml().read(CONFIG_FILE).to(Config.ConfigImpl.class);
            } catch (Exception e) {
            ...
            try {
                for (Field entry : Config.ConfigImpl.class.getFields()) {
                    if (entry.get(impl) == null) {
                        // 字段值为空，使用默认值
                        ModMetadata.LOGGER.warn("Failed to load config entry, resetting to default value: {}", entry.getName());
                        entry.set(impl, Config.ConfigImpl.class.getField(entry.getName()).get(new Config.ConfigImpl()));
                    }
                }
    ...
    /**
     * 将 {@link ConfigManager#impl} 实例中的配置选项的值写入配置文件。若配置文件不存在，则会自动创建。
     *
    ...
    public static void saveConfig() throws IOException {
        ...
        new TomlWriter().write(Objects.requireNonNull(impl), CONFIG_FILE);
    }
}
