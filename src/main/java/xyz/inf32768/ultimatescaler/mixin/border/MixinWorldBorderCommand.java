...
public abstract class MixinWorldBorderCommand {
    ...
    private static float modifyAbs(float value) {
        return ConfigManager.config.fixesAndExpansion.expandWorldBorder ? 0F : Math.abs(value);
    }
    ...
    private static double modifyConstant(double original) {
        return ConfigManager.config.fixesAndExpansion.expandWorldBorder ? 0D : original;
    }
    ...
    private static double modifyConstant2(double original) {
        return ConfigManager.config.fixesAndExpansion.expandWorldBorder ? Double.POSITIVE_INFINITY : original;
    }
}
